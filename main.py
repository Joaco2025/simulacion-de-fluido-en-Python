...
pygame.init()
ancho_pantalla, alto_pantalla = 1100, 600  # Aumentada para espacio de caída
...
while ejecutando:
    ...
    dibujar_texto(f"Velocidad salida: {velocidad_salida_cm_s:.1f} cm/s", 750, 170)
    dibujar_texto(f"Caudal estimado: {caudal_lps:.3f} L/s", 750, 200)
    dibujar_texto(f"Fricción: {'Sí' if friccion else 'No'} (presiona F)", 750, 230)
    dibujar_texto(f"Altura: {Altura_cm:.1f} cm", 750, 260)
    dibujar_texto(f"Partículas en caída: {sum(1 for p in particulas if p.en_caida_libre)}", 750, 290)
